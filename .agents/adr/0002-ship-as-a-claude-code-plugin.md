...
Grok can install this fork as a git plugin:
...
Grok accepts a `skills` array of directory paths (Claude's shape), so the bucket layout does not block it the way Codex is blocked. Two measured constraints on Grok 1.0.40 decide the files:

- `.grok-plugin/plugin.json` overrides `.claude-plugin/plugin.json`. With no `skills` field, discovery walks `skills/` recursively and loads `misc/`, `in-progress/`, and `deprecated/`. The Grok manifest is therefore a byte copy of the Claude manifest, regenerated by `scripts/sync-grok-plugin.mjs`. Paths point at `skills/<bucket>/<name>`, which are real directories. A local `grok plugin install <dir>` drops symlinks. Grok loads those bucket paths.
